...
public class MongoDB2 {
	...
	public static void init(MongoDatabase mdb) {
		// 创建一个集合
		mdb.createCollection("javacol");
		...
		// 插入第一个文档，使用productid为key

		// mdb.getCollection("counters").insertOne(new Document().append("_id",
		// "productid").append("sequence_value", 0));

		// 创建一个函数，指定的序列会自动增长 1 并返回最新序列值
		// int id=getNextSequenceValue("productid", mdb);
	}

	public static void Search(MongoDatabase mdb) {
		// 获取一个集合
		...
		if (collection.count() > 0) {
			// 获取迭代器
			FindIterable<Document> fid = collection.find(); // 获取游标
			...
			while (mcd.hasNext()) {
				...
				System.out.println("tojson: "+doc.toJson());
				//System.out.println("json: "+new JSONObject(doc.toJson()).get("_id"));
	...
	public static void insert(MongoDatabase mdb) {
		...
		Document document = new Document("title", "java MongoDB new").append("content", "使用java插入mongodb数据new")
				.append("showcount", 9);
	...
	public static void incinsert(MongoDatabase mdb) {
	...
	public static void update(MongoDatabase mdb) {
		// 更新文档
		// 获取一个集合
		MongoCollection<Document> collection = mdb.getCollection("javacol");
		// 查询到数据，迭代出数值
		...
		// 将文档中showcount小于20的更新为25
		MongoCollection<Document> collections = mdb.getCollection("javacol");
		// 第一种更新
		// collections.updateMany(Filters.lt("showcount", 20), new
		// Document("$set", new Document("showcount", 25)));
	...
	public static void delete(MongoDatabase mdb) {
		MongoCollection<Document> collections = mdb.getCollection("javacol");
		// 删除文档1
		...
		if (collections.count() > 0) {
			// 获取迭代器
			FindIterable<Document> fid = collections.find();
			// 获取游标
			MongoCursor<Document> mcd = fid.iterator();
			while (mcd.hasNext()) {
				System.out.println(mcd.next());
	...
	public static int getNextSequenceValue(String key, MongoDatabase mdb) {
		Document doc = mdb.getCollection("counters").findOneAndUpdate(new BasicDBObject("_id", key),
				new BasicDBObject("$inc", new BasicDBObject("sequence_value", 1)));
